...
// NOTE: this returns range min queries with range assign updates by default.
// edit the operations here to do what you need it to do
struct LazySegmentTree {
    ...
    void update(int p, int tl, int tr, const int left, const int right,
                const ll val) {
    ...
    // public function to update the values between `left` and `right` by `val`.
    // O(log n) to perform an update
    void update(const int left, const int right, const int val) {
        return update(1, 0, n - 1, left, right, val);
    }
};
